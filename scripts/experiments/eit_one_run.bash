...
screen -dmS kimera $SCRIPT_DIR/../apptainer/exec.bash $SCRIPT_DIR/../run/roslaunch_kimera_k4a.bash left_cam_segmentation_topic:=$method bag_file:=$bag play_bag:=True should_use_sim_time:=True metric_semantic_reconstruction:=true robot_hostname:=szilvi_msi launch_rviz:=false semantic_label_file_path:=/home/szilvi/semantic_mapping/kimera_ws/src/kimera_interface/cfg/categories4detectron2_20.csv

case $method in
    /semantics/superpixel)
        echo "Using supix, launching node..."
        screen -dmS supix $SCRIPT_DIR/../apptainer/exec.bash $SCRIPT_DIR/../run/roslaunch_supix_k4a.bash --wait method:=$1 mesh_path:=$2
        shift 2
        ;;
