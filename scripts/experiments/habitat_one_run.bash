...
if [ $method = rgb ]
then
    screen -dmS kimera $SCRIPT_DIR/../singularity/exec.bash $SCRIPT_DIR/../run/roslaunch_kimera_habitat.bash --wait play_bag:=False launch_rviz:=False metric_semantic_reconstruction:=False
else
    screen -dmS kimera $SCRIPT_DIR/../singularity/exec.bash $SCRIPT_DIR/../run/roslaunch_kimera_habitat.bash --wait left_cam_segmentation_topic:=$method play_bag:=False launch_rviz:=False semantic_label_file_path:=/home/szilvi/semantic_mapping/kimera_ws/src/kimera_interface/cfg/categories4detectron2_20.csv

    case $method in
        /semantics/superpixel)
            echo "Using supix, launching node..."
            screen -dmS supix $SCRIPT_DIR/../singularity/exec.bash $SCRIPT_DIR/../run/roslaunch_supix.bash --wait method:=$1 mesh_path:=$2
            shift 2
            ;;
...
$SCRIPT_DIR/../singularity/exec.bash $SCRIPT_DIR/../run/roslaunch_habitat.bash --wait mesh_path:=$1 sensor_height:=$2 output_agent_pose_name:=$3 depth_noise_multiplier:=$4 replay_mode:=True
